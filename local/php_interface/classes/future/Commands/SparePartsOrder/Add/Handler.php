...
class Handler
{
    ...
    public function handle(Command $command)
    {
        ...
        if ($order) {
            throw new DomainException("Заказ уже создан.", 400);
        }

        $vendorCodeList = array_column($requestElement["PROPERTIES"]["SPARE_PARTS"]["VALUE"], "VENDOR_CODE");
        $sparePartsList = [];
        $arSelect = [];
        $arFilter = [
            "IBLOCK_ID" => iblock_id("catalog:spare-parts"),
            "ACTIVE_DATE" => "Y",
            "ACTIVE" => "Y",
        ...
        foreach ($vendorCodeList as $vendorCode) {
            if (!array_key_exists($vendorCode, $sparePartsList)) {
                throw new Exception("Запчасти с артикулом '$vendorCode' не существует.", 404);
            }
        }
        ...
        $propertyEnums = CIBlockPropertyEnum::GetList(
            ...
            [
                "IBLOCK_ID" => iblock_id("catalog:spare-parts-lk"),
                "CODE" => "STATUS"
            ]
        );
        ...
        foreach ($requestElement["PROPERTIES"]["SPARE_PARTS"]["VALUE"] as $item) {
        ...
        $propertyCollection->getPhone()->setValue($arUser["PERSONAL_PHONE"]);
        $propertyCollection->getPayerName()->setValue($this->user->GetFullName());
        ...
        if ($result->isSuccess()) {
            ...
            $this->CIBlockElement->SetPropertyValuesEx(
                ...
                [
                    "STATUS" => [
                        "VALUE" => $arResult["ENUM_LIST"]["STATUS"]["in_progress"]["ID"]
                    ]
                ]
            );
